...
Intelligent workflow automation engine with 5 integrated subsystems: StateMachine (generic FSM with guards, history, and entry/exit hooks), RuleEngine (forward-chaining with priority ordering, conflict resolution, and audit trails), TaskScheduler (dependency-aware topological execution with failure propagation), Pipeline (event-driven stage orchestrator with middleware and metrics), and CronScheduler (5-field cron parser with next-run calculation). Includes 4 real-world demos (CI/CD, order processing, infrastructure provisioning, job scheduling) and a 23-assertion self-test suite.
